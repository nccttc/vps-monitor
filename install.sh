...
# =================================================================================
# VPS Monitor & Interactive Bot - 统一管理脚本
#
# 功能:
...
install_dependencies() {
    ...
    if [[ "$1" == "bot" ]]; then
        ...
        command_exists pip3 || pkgs+=("python3-pip")
    fi
...
setup_telegram() {
    ...
    if [[ -n "${TG_BOT_TOKEN}" ]]; then
        read -rp "已检测到现有配置，是否覆盖？[y/N]: " overwrite
        [[ ! "${overwrite}" =~ ^[Yy]$ ]] && return 0
    fi

    read -rp "请输入你的 Bot Token: " token
    read -rp "请输入你的 Chat ID: " chat_id
    ...
    if [[ -n "$token" && -n "$chat_id" ]]; then
        ...
        send_telegram "🔔 <b>VPS Monitor 通知配置成功</b>%0A%0A主机: <code>${HOST_NAME}</code>%0AIP: <code>${HOST_IP}</code>"
    else
        log_warn "输入为空，跳过配置。"
    fi
}
...
send_telegram() {
    ...
    if [[ -n "${TG_BOT_TOKEN}" && -n "${TG_CHAT_ID}" ]]; then
        curl -s -X POST "https://api.telegram.org/bot${TG_BOT_TOKEN}/sendMessage" \
            -d chat_id="${TG_CHAT_ID}" -d text="${message}" -d parse_mode="HTML" >/dev/null 2>&1 || true
    fi
}
...
install_monitor() {
    ...
    install_dependencies "exporter"
    setup_telegram # 引导配置TG

    # 安装各组件 (SHA256 校验码建议从官方 release 页面获取并填入)
    install_exporter "node_exporter" "${NODE_EXPORTER_VERSION}" "" "node_exporter" "9100"
    install_exporter "process-exporter" "${PROCESS_EXPORTER_VERSION}" "" "process-exporter" "9256"
...
install_exporter() {
    local name="$1" version="$2" checksum="$3" binary_name="$4" port="$5" args="${6:-}"
    ...
    log_info "正在下载: ${url}"
    if command_exists curl; then curl -sSL -o "${name}.tar.gz" "${url}"; else wget -q -O "${name}.tar.gz" "${url}"; fi
    # if [[ -n "${checksum}" ]]; then echo "${checksum}  ${name}.tar.gz" | sha256sum -c; fi
    tar -xzf "${name}.tar.gz"
...
uninstall_monitor() {
    ...
    log_info "✅ 所有 Exporter 组件已卸载。"
    send_telegram "🗑️ <b>Exporter 监控已卸载</b>%0A%0A主机: <code>${HOST_NAME}</code>%0AIP: <code>${HOST_IP}</code>"
}

restart_monitor() {
    log_info "=== 正在重启所有 Exporter 监控服务 ==="
    systemctl restart node_exporter process-exporter blackbox_exporter
    log_info "✅ 所有 Exporter 服务已重启。"
    send_telegram "🔄 <b>Exporter 监控服务已重启</b>%0A%0A主机: <code>${HOST_NAME}</code>%0AIP: <code>${HOST_IP}</code>"
}
...
install_bot_service() {
    ...
    if [[ ! -f "${CONFIG_FILE}" || -z "${TG_BOT_TOKEN}" ]]; then
        log_warn "未找到 Telegram 配置。请先配置。"
        setup_telegram
        [[ -z "${TG_BOT_TOKEN}" ]] && log_error "Telegram 配置失败，无法安装 Bot。"
    fi

    install_dependencies "bot"
    log_info "正在安装/更新 Python 库: python-telegram-bot"
    pip3 install "python-telegram-bot>=20.0" --upgrade

    log_info "正在创建 Bot 脚本: ${BOT_PY_SCRIPT}"
    cat > "${BOT_PY_SCRIPT}" << 'EOF'
import os
import subprocess
import logging
...
BOT_TOKEN = os.getenv("VPS_BOT_TOKEN")
ALLOWED_CHAT_ID = os.getenv("VPS_CHAT_ID")

if not (BOT_TOKEN and ALLOWED_CHAT_ID):
    logger.error("环境变量 VPS_BOT_TOKEN 或 VPS_CHAT_ID 未设置!")
    exit(1)

try:
    admin_filter = filters.User(user_id=int(ALLOWED_CHAT_ID))
except ValueError:
    logger.error("环境变量 VPS_CHAT_ID 不是一个有效的整数!")
    exit(1)
...
async def status_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        mem_info = "\n".join(mem_output.splitlines()[:2])
        disk_output = subprocess.check_output("df -h /", shell=True).decode("utf-8")
        disk_info = disk_output.splitlines()[1]
        
        message = (
            f"<b>📊 主机 <code>{hostname}</code> 状态报告</b>\n\n"
        ...
        logger.error(f"执行 status 命令失败: {e}")
        await update.message.reply_text("获取服务器状态时出错，请检查服务器日志。")
...
EOF
    ...
    log_info "正在创建 systemd 服务: ${BOT_SERVICE_FILE}"
...
EOF
    ...
    log_info "✅ 交互式 Bot 服务安装/更新成功并已启动！"
    log_info "请在 Telegram 中向你的机器人发送 /status 命令进行测试。"
}
...
uninstall_bot_service() {
    ...
    rm -f "${BOT_SERVICE_FILE}" "${BOT_PY_SCRIPT}"
    systemctl daemon-reload
    log_info "✅ 交互式 Bot 服务已卸载。"
}

restart_bot_service() {
    log_info "=== 正在重启交互式 Bot 服务 ==="
    systemctl restart vps-bot.service
    log_info "✅ 交互式 Bot 服务已重启。"
}

view_bot_logs() {
    log_info "=== 查看 Bot 服务日志 (按 Ctrl+C 退出) ==="
    journalctl -u vps-bot.service -f -n 50
}
...
show_menu() {
    ...
    echo "  1. 安装 Exporter (Install Exporters)"
    echo "  2. 卸载 Exporter (Uninstall Exporters)"
    echo "  3. 重启 Exporter (Restart Exporters)"
    echo -e "${C_YELLOW}--- 交互式 Bot ---${C_RESET}"
    echo "  4. 安装/更新 Bot 服务 (Install/Update Bot Service)"
    echo "  5. 卸载 Bot 服务 (Uninstall Bot Service)"
    echo "  6. 重启 Bot 服务 (Restart Bot Service)"
    echo "  7. 查看 Bot 日志 (View Bot Logs)"
    echo -e "${C_YELLOW}--- 其他 ---${C_RESET}"
    echo "  8. 重新配置 Telegram (Re-configure Telegram)"
    echo "  9. 退出 (Exit)"
    echo "----------------------------------------------------"
...
main() {
    ...
    if [[ $# -gt 0 ]]; then
        case "$1" in
            ...
            install_bot) install_bot_service ;;
            uninstall_bot) uninstall_bot_service ;;
            restart_bot) restart_bot_service ;;
            *) log_error "无效参数: $1。有效参数: install, uninstall, restart, install_bot, uninstall_bot, restart_bot" ;;
        esac
    else
        while true; do
            show_menu
            case "${choice}" in
                ...
                2) uninstall_monitor ;;
                3) restart_monitor ;;
                4) install_bot_service ;;
                5) uninstall_bot_service ;;
                6) restart_bot_service ;;
                7) view_bot_logs ;;
                8) setup_telegram ;;
                9) echo "脚本已退出。"; exit 0 ;;
